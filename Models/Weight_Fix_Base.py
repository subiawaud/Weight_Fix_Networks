...
LAYERS_PRUNED = (nn.Linear, nn.Conv1d, nn.Conv2d, nn.Conv3d)
...
class Weight_Fix_Base(pl.LightningModule):
    ...
    def __init__(self):
        ...
        self.reference_image = None # will hold an example image from the dataset
        self.fixing_iteration = 0 # which iteration of the fixing algorithm are we at
        self.name = 'Base'  # object name
        self.outer_logger = None # logger over entire fixing iterations
        self.optim = None  # torch optimiser
        self.scheduler = None  # torch optimisation scheduler
        self.percentage_fixed = 0 # how many of the weights need to be fixed to a cluster
        self.cluster_bit_fix = 32 # the bit precision of the clusters
        self.hist_epoch_log = 10 # how often do we capture the log metrics in tensorboard
        self.distance_change = 0.01 # what is the change in distance allowed for clusters to be moved between each fixing iteration
        self.end_distance = 0.01 # what is the final distance allowed for weights to be moved to a cluster (final iteration)
        self.clusters = 1 # number of current cluster centroids
        self.iterations = 10 # how many weight fix iterations will be ran
        self.is_fixed = []
        self.t = 1
...
#        self.register_parameter('fixed_centers', nn.Parameter(self.fixed_center_list)))
        self.t = t
        self.gamma = gamma
    ...
    def set_up_fixed_weight_array(self):
        ...
        for n,p in self.named_parameters():
            zeros = torch.zeros_like(p.data.detach()).to(self.device)
            self.fixed_weights.append(zeros)
            self.is_fixed.append(np.array(zeros > 0))
            self.grads.append(zeros)
    ...
    def reset_weights(self):
        for i, (n, p) in enumerate(self.named_parameters()):
            ...
            if 'final' in n:
                ...
                p.data[np.where(self.is_fixed[i])] = torch.Tensor(new_param).to(self.device) # torch.Tensor(new_param).to(self.device)#.flatten()
            with torch.no_grad():
    ...
    def get_cluster_assignment_prob(self):
        ...
        weighted = torch.mean(torch.abs(s * distances), axis =1)
        normalised = torch.div(weighted, torch.abs(weights))
        loss_term = torch.mean(normalised)
        return loss_term
    ...
    def flatten_grads(self):
       return np.concatenate([i.flatten() for i in self.grads])
    ...
    def assign_to_clusters(self, weights, percentage):
...
#        distance_allowed = self.end_distance + (self.iterations - self.fixing_iteration)*self.distance_change
        ...
        closest_distance = np.min(distances, axis = 1) / np.abs(weights) # what is the distance to the closest centroid for each weight
        print(closest_distance)
#        closest_distance += self.grad_weight * self.flatten_grads()
        #print(self.flatten_grads())
        #print(closest_distance)
        if percentage != 1.0:
            idx = self.select_layer_wise(closest_distance, distance_allowed, percentage)
        else:
           idx = range(number_fixed)
        val = np.abs(np.mean(closest_distance[idx])) # + np.std(closest_distance[idx])
        print('val is ', val)
        if val > distance_allowed:
             self.clusters += 1
             return self.assign_to_clusters(weights, percentage)
        ...
        self.clusters = len(np.unique(weights[idx]))
    ...
    def cluster_prune(self, clusters):
        count = 0
        weights, idx, clustered = self.assign_to_clusters(self.flatten_the_network(), self.percentage_fixed)
        fixed = 0
        for i, s in enumerate(self.layer_shapes):
            start = count
            count += np.prod(s)
    ...
    def on_after_backward(self):
        for i, (n, v) in enumerate(self.named_parameters()):
             ...
             if self.current_epoch == self.max_epochs-1:
                 self.grads[i] = np.abs(v.grad.data.cpu().detach().numpy())

    def convert_to_pows_of_2(self, centroids):
         import math
         for i,x in enumerate(centroids):
             if x == 0:
                 continue
             if x < 0:
                 centroids[i] = -2**(math.ceil(math.log2(abs(x))))
             else:
                 centroids[i] = 2**(math.ceil(math.log2(x)))
         return centroids
#        mindiff =


    def convert_to_add_pows_of_2(self, centroids):
        first = self.convert_to_pows_of_2(centroids)
        diff = centroids - first
        second = self.convert_to_pows_of_2(diff)
        return first + second
    ...
    def round_to_precision(self, centroids):
        if self.cluster_bit_fix == '32':
           ...
           return self.convert_to_pows_of_2(centroids)
        elif self.cluster_bit_fix == 'pow_2_add':
           return self.convert_to_add_pows_of_2(centroids)
        else:
           raise ValueError
    ...
    def get_centroids(self, weights, clusters, distance):
        ...
        digitized = self.round_to_precision(digitized / (np.histogram(weights, bins)[0]+e))
        digitized = np.append([0], digitized)
        print(digitized)
        selected = np.unique(digitized[idx[1:]]) # we take all but the last to be our clusters

        if self.encourage_plus_one_cluster:
            self.fixed_center_list = torch.Tensor([np.unique(digitized[idx])]).to(self.device)
