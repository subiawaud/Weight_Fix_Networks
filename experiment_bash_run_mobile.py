...
limits = ["0.001 0.0005 0.00025 0.075"]
changes = ["0.0001 0.0002 0.00005"]
first_epochs = [40]
rest_epochs = [20, 5, 10]
bits = [32]
...
for param_set in params:
         ...
         script_test =f'--encourage_plus_one_cluster {epoc} --t {t} --gamma {gamma} --limits_to_try {limit} --change_to_try {c} --percentages {percentage} --first_epoch {f_e} --fixing_epochs {r_e} --bits {b} --name "mb_net_exp1" --model "mobilenet"'
         os.system('sbatch script_mobile.sh ' + script_test)
